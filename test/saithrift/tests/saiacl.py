...
class L3AclTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        """
        Create an Layer3 ACL and one ACL entry to deny SIP and SPORT received on a router interface.
        Steps:
        1. create two router interfaces
        2. create route
        3. send packet from port2 to port1 and verify packet on port1
        4. create ACL table, entry and bind to rif1 OID
        5. send packet from port2 to port1 and verify that packet from SIP and SPORT is not received on port1
        6. clean up.
        """
        print
        print "Sending packet port 2 -> port 1 (192.168.100.100 -> 10.10.10.1 [id = 105])"
        switch_init(self.client)
        port1 = port_list[1]
        port2 = port_list[2]
        L4_SRC_PORT = 1000
        ...
        # setup mandatory attributes on creation ACL table
        stage = SAI_ACL_STAGE_INGRESS
        bind_point = SAI_ACL_BIND_POINT_ROUTER_INTF
        table_priority = SAI_SWITCH_ATTR_ACL_TABLE_MINIMUM_PRIORITY
